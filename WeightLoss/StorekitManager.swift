...
class StoreKitManager: ObservableObject {
    ...
    func updateCustomerProductStatus() async {
        ...
        for await result in Transaction.currentEntitlements {
            do {
                ...
                // since we only have one type of producttype - .nonconsumables -- check if any storeProducts matches the transaction.productID then add to the purchasedProducts
    ...
    func isPurchased(_ product: Product) async throws -> Bool {
        //as we only have one product type grouping .nonconsumable - we check if it belongs to the purchasedProducts which ran init()
